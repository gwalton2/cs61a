...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        """
        if insect.is_ant and (not isinstance(insect, QueenAnt) or insect.is_imposter): #insures a Queen is not removed but an imposter Queen is. 
        ...
        if not isinstance(insect, QueenAnt) or insect.is_imposter:  #makes sure a Queen's place is not none. 
            insect.place = None
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        place_num = 0
        if self.min_range:
            while place_num < self.min_range: #makes sure the program searches past the minimum range for LongThrower
                bee_place = bee_place.entrance
                place_num += 1 
        while not bee_place.bees and bee_place != hive and place_num is not self.max_range: #Searches for a bee until the it hits the hive or the maximum range for ShortThrower
            bee_place = bee_place.entrance
            place_num += 1
        if bee_place is hive:
            return None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            ...
            while len(bees) > 0:    #iterates through the list of bees in self.place
                bee = bees[0]
                bees = bees[1:]
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 6A
        bees = self.place.bees
        while len(bees) > 0:    #iterates through a list of bees in its place using the same method as FireAnt
            bee = bees[0]
            bees = bees[1:]
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    # BEGIN Problem 9
    food_cost = 7
    number_queens = 0   # Variable to keep track of how many QueenAnt instances are currently active
    implemented = True   # Change to True to view in the GUI
    # END Problem 9
    ...
    def __init__(self):
        ...
        QueenAnt.number_queens += 1 #Adds one to class attribute as another QueenAnt instance has been created
        if self.number_queens > 1:  #If there is one Queen already than this instance is an imposter
    ...
    def action(self, colony):
        ...
        if self.number_queens > 1:
            QueenAnt.number_queens -= 1
            self.reduce_armor(self.armor)
        else:
            while exit:
                if exit.ant:
                    if hasattr(exit.ant, 'container') and exit.ant.container and exit.ant.ant and exit.ant.ant not in self.ants: #Checks if the ant is containing another ant.
                        exit.ant.ant.damage *= 2
                        self.ants.append(exit.ant.ant)  #Adds the ant to its list so it knows not to double its damage again.
                    if exit.ant not in self.ants:
                        exit.ant.damage *= 2
                        self.ants.append(exit.ant)
                exit = exit.exit
