...
def roll_dice(num_rolls, dice=six_sided):
    ...
    score=0
    pig_out=0 
    for curr_num_rolls in range(0,num_rolls):
        roll = dice()
        if roll==1:
            pig_out+=1
        else:
            score = score + roll
    if pig_out:
        return min(pig_out,11-num_rolls)
    else:
        return score
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    "*** REPLACE THIS LINE ***"
    if num_rolls==0:
        turn = free_bacon(opponent_score)
    else:
        turn = roll_dice(num_rolls, dice)
    if is_prime(turn):
        return next_prime(turn)
    else:
        return turn
    # END PROBLEM 2
...
def check_strategy(strategy, goal=GOAL_SCORE):
    ...
    for score in range(0, goal):
        for opponent_score in range(0, goal):
            check=strategy(score,opponent_score)
            if check>10 or check<-1:
                return check_strategy_roll(score,opponent_score,check)
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    best_avg=average(1,dice)
    for numroll in range(2,11):
        if average(numroll,dice)>best_avg:
            best_avg,best_num=average(numroll,dice),numroll
    return best_num
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if score>opponent_score+10:
        margin,roll=6,5
    elif score<opponent_score-4:
        margin,roll=11,8
    if dice==four_sided:
        margin,roll=5,4
    if score+points==opponent_score*2 :
        return roll
    return swap_strategy(score, opponent_score, margin, roll)
