...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        check_procedure(val)
        return Procedure.eval_call(val, expr.second, env)
...
def eval_all(expressions, env):
    ...
    if expressions == nil:
        return None
    exprs = expressions.map(lambda expr: scheme_eval(expr, env))
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if not (vals is nil and formals is nil):
            raise SchemeError
        # END PROBLEM 11
        return child
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        operands = operands.map(lambda oper: scheme_eval(oper, env))
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        symbol = scheme_eval(expressions.second.first, env)
        env.define(target, symbol)
        return target
        # END PROBLEM 6
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 10
        new_target = target.first
        symbol = LambdaProcedure(target.second, expressions.second, env)
        env.define(new_target, symbol)
        return new_target
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    while scheme_truep(expr) and expressions.second is not nil:
...
def do_or_form(expressions, env):
    ...
    if expressions == nil:
        return False
    expr = scheme_eval(expressions.first, env)
    while not scheme_truep(expr) and expressions.second is not nil:
        expr = scheme_eval(expressions.second.first, env)
        expressions = expressions.second
    if scheme_truep(expr):
        return expr
